...
public class SqliteSeeder implements Seeder {
    ...
    public void seed() throws SQLException {
        ...
        try (Connection conn = factory.getConnection(); Statement stmnt = conn.createStatement()) {
            conn.setAutoCommit(false);

            String[] queries = Arrays.stream(query.split(";")).filter((s) -> !s.isBlank()).map((s) -> s.trim())
                    .toArray(String[]::new);
