...
public class SignUpAuthScene implements Scene {
	...
	private EnterKeyListener inputKeyEnterListener = new EnterKeyListener(new EnterKeyCallback() {
		@Override
	...
	private boolean validateLastName() {
		...
		if (lastName.length() < StringLimits.LAST_NAME.min() || lastName.length() > StringLimits.LAST_NAME.max()) {
			SwingUtilities.invokeLater(() -> {
				lastNameInputError.setText(HtmlUtils.wrapInHtml(String.format("Display name must be >= %s and <= %s",
	...
	private boolean validateFirstName() {
		...
		if (firstName.length() < StringLimits.FIRST_NAME.min() || firstName.length() > StringLimits.FIRST_NAME.max()) {
			SwingUtilities.invokeLater(() -> {
				firstNameInputError.setText(HtmlUtils.wrapInHtml(String.format("Display name must be >= %s and <= %s",
	...
	private boolean validateDisplayName() {
		...
		if (displayName.length() < StringLimits.DISPLAY_NAME.min()
			...
			SwingUtilities.invokeLater(() -> {
				displayNameInputError.setText(HtmlUtils.wrapInHtml(String.format("Display name must be >= %s and <= %s",
	...
	private void signUp(boolean shouldValidate) {
		...
		try {
			if (shouldValidate && (!validateEmail() | !validateDisplayName() | !validateFirstName()
			...
			SwingUtilities.invokeLater(() -> {
				JOptionPane.showMessageDialog(view, "You have been registered successfully. Please login.", "Success",
	...
	public void onCreate() {
		view.setLayout(new MigLayout("insets 0, gapy 28px, flowy, fillx", "[grow,center]", "[grow,center]"));
		...
		formContainer.setLayout(new MigLayout("insets 0, gapy 16px, flowy, fillx", "[grow,center]", "[center]"));
		inputFormContainer.setLayout(new MigLayout("insets 0, gapy 9px, flowy, fillx", "[grow,center]", "[center]"));

		emailDisplayNameInputContainer
				.setLayout(new MigLayout("insets 0, gapx 4px, flowx", "[grow,center]", "[center]"));
		nameInputContainer.setLayout(new MigLayout("insets 0, gapx 4px, flowx, fillx", "[grow,center]", "[center]"));
		...
		displayNameInputContainer.setLayout(new MigLayout("insets 0, flowy, fillx", "[grow,center]", "[center]"));
		...
		firstNameInputContainer.setLayout(new MigLayout("insets 0, flowy, fillx", "[grow,center]", "[center]"));
		lastNameInputContainer.setLayout(new MigLayout("insets 0, flowy, fillx", "[grow,center]", "[center]"));
