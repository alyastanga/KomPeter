...
public interface Inventory {
	...
	// purchasing methods
	void addPurchaseItem(int supplierId, Timestamp purchaseDate, String code, @Nullable Timestamp deliveryDate,
			double vat, double discVal, DiscountType discType);

	void addPurchasePayments(int _purchaseId, Timestamp paymentDate, String refNumber, PaymentMethod paymentMethod,
			double amount);

	void addPurchaseItemStocks(int _purchaseId, int _itemStocksId, String refNumber, PaymentMethod paymentMethod,
			double amount);

	// Assignment methods
	void setItemStockStorageLoc(int _itemStockId, int quantity_ordered, int quantity_received, int quantity,
			double unitCost);
