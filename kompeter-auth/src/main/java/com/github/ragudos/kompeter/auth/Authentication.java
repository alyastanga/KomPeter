...
public final class Authentication {
	...
	public static void signInFromStoredSessionToken() throws AuthenticationException {
		...
		try (Connection conn = factory.getConnection();) {
			...
			SessionDto sessionDto = sessionDao.getSessionByToken(conn, sessionToken)
	...
	public static void signIn(final @NotNull String email, final @NotNull char[] password)
		...
		try (Connection conn = factory.getConnection();) {
			...
			AccountPassword accountPassword = accountDao.getAccountPassword(conn, email)
					.orElseThrow(() -> new AuthenticationException(AuthenticationErrors.INVALID_CREDENTIALS));
			HashedStringWithSalt hashedPassword = Hasher.hash(password, Salt.fromBase64(accountPassword.passwordSalt()))
					.orElseThrow(AuthenticationException::new);
			...
			UserDto userDto = userDao.getUserByEmail(conn, email).orElseThrow(AuthenticationException::new);
			...
			try {
				...
				SessionDto sessionDto = sessionDao.getSessionById(conn, _sessionId)
						.orElseThrow(AuthenticationException::new);

				UserMetadataDto userMetadataDto = new UserMetadataDto(userDto._userId(), userDto._createdAt(),
						userDto.displayName(), userDto.firstName(), userDto.lastName(), userRoleDao
								.getRolesOfUserById(conn, userDto._userId()).orElseThrow(AuthenticationException::new),
						email);
	...
	public static void signUp(final @NotNull String displayName, final @NotNull String firstName,
			...
			final @NotNull char[] confirmPassword) throws AuthenticationException {
		...
		try (Connection conn = factory.getConnection();) {
			...
			HashedStringWithSalt hashedPassword = Hasher.hash(password).orElseThrow(AuthenticationException::new);
